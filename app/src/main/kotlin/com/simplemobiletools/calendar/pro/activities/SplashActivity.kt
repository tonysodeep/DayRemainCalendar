...
import com.simplemobiletools.calendar.pro.GetStartedActivity
...
class SplashActivity : BaseSplashActivity() {
    override fun initActivity() {
        when {
            intent.extras?.containsKey(DAY_CODE) == true -> Intent(this, GetStartedActivity::class.java).apply {
                putExtra(DAY_CODE, intent.getStringExtra(DAY_CODE))
                putExtra(VIEW_TO_OPEN, intent.getIntExtra(VIEW_TO_OPEN, LAST_VIEW))
                startActivity(this)
            }
            intent.extras?.containsKey(EVENT_ID) == true -> Intent(this, GetStartedActivity::class.java).apply {
            ...
            intent.action == SHORTCUT_NEW_EVENT -> {
                ...
                Intent(this, GetStartedActivity::class.java).apply {
                    putExtra(NEW_EVENT_START_TS, getNewEventTimestampFromCode(dayCode))
                    startActivity(this)
                }
            }
            else -> startActivity(Intent(this, GetStartedActivity::class.java))
        }
        finish()
    }
}
